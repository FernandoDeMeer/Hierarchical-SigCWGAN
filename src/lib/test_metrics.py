...
def Phi(X, order, normalise=True, compute_sigs=True):
    if compute_sigs:
        dim = np.shape(X)[1]
        sig = signatory.signature(path= X.unsqueeze(dim=0), depth = order)
    else:
        dim = 2
        sig = np.array(X)
    ...
    keys = signatory.all_words(dim, order)

    phi_x = phi(tuple(sig[0]), order)

    Lambda = np.array([phi_x ** len(t) for t in keys])

    return Lambda * np.array(sig[0])


def T(set1, set2, order, verbose=True, normalise=False, compute_sigs=True):
    m = len(set1)
    n = len(set2)
...
def test(set1, set2, order, confidence_level=0.9999, **kwargs):
