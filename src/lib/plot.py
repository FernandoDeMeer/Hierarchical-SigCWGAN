...
from src.lib.test_metrics import *
...
def plot_summary(x_fake, x_real, max_lag=None, labels=None):
    ...
    from src.lib.test_metrics import skew_torch, kurtosis_torch
    dim = x_real.shape[2]
...
def compare_cross_corr(x_real, x_fake, GAN_type):
    ...
    axes[1].set_title('{}'.format(GAN_type))
...
def plot_matrix(dataset, matrix, experiment_directory, method):
    ...
    multi_asset_data.index = pd.to_datetime(multi_asset_data.index)
    f = plt.figure(figsize=(19, 15))
    plt.matshow(matrix, fignum=f.number,vmin=0, vmax=1)
    plt.xticks(range(multi_asset_data.select_dtypes(['number']).shape[1]), multi_asset_data.select_dtypes(['number']).columns, fontsize=14, rotation=90)
    plt.yticks(range(multi_asset_data.select_dtypes(['number']).shape[1]), multi_asset_data.select_dtypes(['number']).columns, fontsize=14)
    cb = plt.colorbar()
    cb.ax.tick_params(labelsize=14)
    plt.title('{} Matrix'.format(method), fontsize=16);
    plt.savefig(experiment_directory + '/{}_matrix.png'.format(method))
    plt.clf()
