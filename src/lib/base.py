...
from src.lib.utils import to_numpy
...
class BaseAlgo:
    def __init__(self, base_config, x_input, x_output):
        ...
        self.input_dim = self.latent_dim = x_input.shape[-1]
        self.output_dim = x_output.shape[-1]

        self.training_loss = defaultdict(list)
        self.test_metrics_list = get_standard_test_metrics(x_input)

        self.G = SimpleGenerator(self.p * self.input_dim, self.output_dim, self.hidden_dims, self.latent_dim).to(
            self.device)

    def fit(self):
        if self.batch_size == -1:
            self.batch_size = self.x_input.shape[0]
        for _ in tqdm(range(self.total_steps),position=0,leave=True):  # sig_pred, x_past, x_real
            self.step()
...
class CrossDimAlgo:
    def __init__(self, cross_dim_config, x_input, x_output, x_input_base, base_sigcwgan):
        self.base_config = cross_dim_config
        self.batch_size = cross_dim_config.batch_size
        self.hidden_dims = cross_dim_config.hidden_dims
        self.p = cross_dim_config.p
        self.q = cross_dim_config.q
        self.total_steps = cross_dim_config.total_steps

        self.device = cross_dim_config.device

        self.x_input = x_input
        self.x_output = x_output
        self.x_input_base = x_input_base
        # The input are the base_dims
        self.input_dim = x_input_base.shape[-1]
        # The output are the target_dims
        self.output_dim = x_input.shape[-1] - x_input_base.shape[-1]

        self.training_loss = defaultdict(list)
        # self.test_metrics_list = get_standard_test_metrics(x_input)

        self.G = CrossDimGenerator(self.p*self.input_dim, self.q*self.output_dim,
                                   self.hidden_dims,).to(self.device)

        self.base_G = base_sigcwgan.G
    ...
    def plot_losses(self):
        ...
        axes.plot(np.convolve(loss, np.ones(100)/100, mode='valid')
                  , label='Sig-$W_1$ loss')
        axes.set_yscale('log')
        axes.grid()
        axes.legend()
        axes.set_xlabel('CrossDim-Generator weight updates')
